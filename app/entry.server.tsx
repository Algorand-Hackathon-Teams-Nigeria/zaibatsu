...
import { ApolloProvider, ApolloClient, InMemoryCache, createHttpLink } from "@apollo/client/index.js";
...
function handleBrowserRequest(request: Request, responseStatusCode: number, responseHeaders: Headers, remixContext: EntryContext) {
  return new Promise(async (resolve, reject) => {
    ...
    const { pipe, abort } = renderToPipeableStream(
      await wrapRemixServerWithApollo(<RemixServer context={remixContext} url={request.url} abortDelay={ABORT_DELAY} />, request),
      {
        onShellReady() {
          shellRendered = true;
          const body = new PassThrough();
...
async function getApolloClient(request: Request) {
  const client = new ApolloClient({
    ...
    link: createHttpLink({
      uri: "https://zaibatsubackend-production.up.railway.app/graphql",
      headers: {
