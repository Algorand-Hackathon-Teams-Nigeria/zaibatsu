...
const SidebarNavigation = () => {
  ...
  return (
    <aside
      className={`md:relative transition-all top-14 fixed h-screen pl-14 bg-[#E9FCF5] ${isMobile && isOpen
          ? "translate-x-0"
          : isMobile && !isOpen
            ? "-translate-x-full"
            : ""
        ...
        <ul className="flex flex-col bg-white">
          {DAPP_NAVIGATION.map((item, id) => (
            <div key={item.name} className="group transition-all">
              <div
                className={`h-5 w-full bg-[#E9FCF5] z-0 ${active === id ? "rounded-br-3xl" : ""
                  } top-[-12px] right-0`}
              />
              <Link
              ...
              >
                <span
                  className={`p-3 px-14 pr-16 rounded-l-3xl group-hover:text-primary/50 transition-all ${active === id ? "bg-white text-primary" : "bg-[#E9FCF5]"
                    } w-full h-full`}
                >
                  {item.name}
                </span>
              ...
              <div
                className={`h-5 w-full bg-[#E9FCF5] z-0 ${active === id ? "rounded-tr-3xl" : ""
                  } top-[-12px] right-0`}
              />
            </div>
          ))}
